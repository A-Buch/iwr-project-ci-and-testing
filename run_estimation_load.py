...
## load origignal parameters file (not interpolated)
...
for n in run_numbers[:]:
    ...
    outdir_for_cell = dh.make_cell_output_dir(
        #s.output_dir, "timeseries", sp["lat"], sp["lon"], s.variable   # ts of origignal parameters file (not interpolated)
        s.output_dir, "timeseries_interpolated", sp["lat"], sp["lon"], s.variable  # ts of interpolated parameters
    )
    ...
    if s.skip_if_data_exists:
        try:
            ...
            print(f"Existing valid parameter data in {fname_cell} . Skip calculation.")
            continue
